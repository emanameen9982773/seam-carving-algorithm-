 ...
 *  2-findMinSeam() which is the core of seam carving, this method search for a verticl path having the minimum sum of energies by exploring all possible paths from the bottom pic to the top of it
 *  3-removeSeam() it remove the seam by copying the RBG of each pixel in the original image into the resized one skiping the unwanted pixels from each row
 */

public class SeamCarving1 {

   static int min=Integer.MAX_VALUE;           // stores the minimum sum of energies
   static LinkedList<Integer> pathOfMinSeam ; //  stores the path of the seam with minimum sum of energies
   ...
   public static int[][] calculatepixelsEnergy(BufferedImage image){ 
    ...
    public static void findMinSeam(int[][] pixelsEnergy, int sumOfEnergies, int column, int row, LinkedList<Integer> path){
        ...
        path.add(column);                                        // add the number of the column for current pixel to keep track of the path
        sumOfEnergies+=pixelsEnergy[row][column];               // add the energy of current pixel to compare later the sum of energeies of this path
        if(row==0){                                            // reached the top of matrix 
            if(sumOfEnergies<min){                         
             min=sumOfEnergies;                              // the process of choosing the minimum energy seam and store its path in global varaible "pathOfMinSeam"              
             pathOfMinSeam=new LinkedList<>(path);
            }
            return;
        } // recursive calls
        findMinSeam(pixelsEnergy,sumOfEnergies,column,row-1,new LinkedList<>(path)) ;   // exploring the stright path without going left or right  *column*
        findMinSeam(pixelsEnergy,sumOfEnergies,column+1,row-1,new LinkedList<>(path)) ; // exploring the path to the right   *column+1*
        findMinSeam(pixelsEnergy,sumOfEnergies,column-1,row-1,new LinkedList<>(path)) ; // exploring the path to the left    *column-1*    
